...
@st.cache_data
def recommender_cf(liked_books, streamlit_df):
    # Create data frame of unique users who like the same books as user
    filtered_ratings = streamlit_df[streamlit_df['isbn'].isin(liked_books) & (streamlit_df['individual_rating'] > 8)]
    overlap_users = set()
    ...
    total_books = streamlit_df[streamlit_df['user_id'].isin(overlap_users['user_id'])]
